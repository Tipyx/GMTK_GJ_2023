...
func init(c:Character, q:Quest):
	if c.charac_class == JobData.Jobs.WARRIOR and q.quest_enviro == EnviroData.Biomes.SWAMP:
		desc_label.text = "Venturing deep into the swamp requires agility, wearing armour is not an advantage."
	elif c.charac_class == JobData.Jobs.WARRIOR and q.quest_type == QuestData.Quests.ROBBERY:
		desc_label.text = "Robbery requires stealth. Have you ever see a warrior being inconspicuous?"
	elif c.charac_class == JobData.Jobs.WARRIOR and q.quest_enviro == EnviroData.Biomes.CAVE:
	...
	elif c.charac_class == JobData.Jobs.THIEF and q.quest_enviro == EnviroData.Biomes.DESERT:
	...
	elif c.charac_class == JobData.Jobs.PEASANT and q.quest_type == QuestData.Quests.ROBBERY:
	...
	elif c.charac_class == JobData.Jobs.PEASANT and q.quest_monster == MonsterData.Monsters.CATS:
	...
	elif c.charac_class == JobData.Jobs.DRUID and q.quest_monster == MonsterData.Monsters.WOLVES:
		desc_label.text = "There is one thing that druids love more than anything else: animals. All of them."
	elif c.charac_class == JobData.Jobs.DRUID and q.quest_monster == MonsterData.Monsters.CATS:
		desc_label.text = "Druids are really great at two things: picking flowers & cuddle furred animal."
	elif c.charac_class == JobData.Jobs.DRUID and q.quest_monster == MonsterData.Monsters.SNAKES:
		desc_label.text = "Druids are good company, even for snakes."
	elif c.charac_class == JobData.Jobs.DRUID and q.quest_monster == MonsterData.Monsters.SPIDERS:
	...
	elif c.charac_specie == SpecieData.Species.HUMAN and q.quest_monster == MonsterData.Monsters.CHILDS:
		desc_label.text = "Most humans think childs are the cutest thing in the world. Humans are weird."
	elif c.charac_specie == SpecieData.Species.HUMAN and q.quest_monster == MonsterData.Monsters.CATS:
		desc_label.text = "Cats learned to meow to touch humans. And it worked well."
	elif c.charac_specie == SpecieData.Species.ORK and q.quest_monster == MonsterData.Monsters.ORKS:
		desc_label.text = "Orks are almost all cousins, it's just one big family."
	elif c.charac_specie == SpecieData.Species.ORK and q.quest_enviro == EnviroData.Biomes.DESERT:
		desc_label.text = "One of the biggest enemies of orks is the heat. Then it's Dwarfs. "
	elif c.charac_specie == SpecieData.Species.ORK and q.quest_enviro == EnviroData.Biomes.SWAMP:
	...
	elif c.charac_specie == SpecieData.Species.ORK and q.quest_type == QuestData.Quests.ROBBERY:
		desc_label.text = "Orks are not well known for their discretion, which is inversely proportional to their fighting skills."
	elif c.charac_specie == SpecieData.Species.HALFLING and q.quest_type == QuestData.Quests.DELIVERY:
		desc_label.text = "It's not easy to lift a package which is twice your weight."
	elif c.charac_specie == SpecieData.Species.HALFLING and q.quest_monster == MonsterData.Monsters.WOLVES:
		desc_label.text = "My grandpa told me that a Wolf's favourite meal is Halflings. Not sure if it's true though."
	elif c.charac_specie == SpecieData.Species.HALFLING and q.quest_monster == MonsterData.Monsters.CARNIVOROUS_PLANTS:
	...
	elif c.charac_specie == SpecieData.Species.UNICORN and q.quest_type == QuestData.Quests.ROBBERY:
		desc_label.text = "Did you try to pick up something with a hoof? You can't, actually."
	elif c.charac_specie == SpecieData.Species.UNICORN and q.quest_enviro == EnviroData.Biomes.SEA:
		desc_label.text = "Fire burns, water wets and unicorns sink."
	elif c.charac_specie == SpecieData.Species.UNICORN and q.quest_enviro == EnviroData.Biomes.JUNGLE:
		desc_label.text = "Unicorns are way better at carrying people and objects than walking in the middle of creepers."
	elif c.charac_specie == SpecieData.Species.UNICORN and q.quest_enviro == EnviroData.Biomes.CAVE:
		desc_label.text = "It's not easy for unicorns to evolve in tight spaces."
	elif c.charac_specie == SpecieData.Species.CAT and q.quest_monster == MonsterData.Monsters.CATS:
		desc_label.text = "Did you notice that cats were working together to bring chaos & desolation to our land?"
	elif c.charac_specie == SpecieData.Species.CAT and q.quest_enviro == EnviroData.Biomes.SEA:
		desc_label.text = "If you want to scare away a cat, throw water."
	elif c.charac_specie == SpecieData.Species.CAT and q.quest_enviro == EnviroData.Biomes.SWAMP:
		desc_label.text = "Cats hate water as much as they love children because they are too young to exercise caution."
	elif c.charac_specie == SpecieData.Species.ELF and q.quest_monster == MonsterData.Monsters.ELVES:
	...
	elif c.charac_specie == SpecieData.Species.ELF and q.quest_enviro == EnviroData.Biomes.CAVE:
	...
	elif c.charac_specie == SpecieData.Species.DWARF and q.quest_monster == MonsterData.Monsters.DWARVES:
		desc_label.text = "While Dwarfs should fight against other dwarfs when they can fight Elves & Orcs?"
	elif c.charac_specie == SpecieData.Species.DWARF and q.quest_type == QuestData.Quests.PICKING:
		desc_label.text = "Even though they are close to the ground, it's pretty painful for a Dwarf to bend over"
	elif c.charac_specie == SpecieData.Species.DWARF and q.quest_enviro == EnviroData.Biomes.SEA:
		desc_label.text = "Dwarves are not sailors, they belong to rocks."
	elif c.charac_specie == SpecieData.Species.DWARF and q.quest_enviro == EnviroData.Biomes.FOREST:
		desc_label.text = "Why would a dwarf decide to go into the forest when they can stay in a cave?"
	elif c.charac_trait == TraitData.Traits.CLAUSTROPHOBIC and q.quest_enviro == EnviroData.Biomes.CAVE:
		desc_label.text = "You sent a claustrophobic person into a cave. Do you need more information, boss?"
	elif c.charac_trait == TraitData.Traits.POLLEN_ALLERGY and q.quest_enviro == EnviroData.Biomes.FOREST:
		desc_label.text = "If I am correct, you could find flowers & trees in a forest. They both spread pollen."
	elif c.charac_trait == TraitData.Traits.POLLEN_ALLERGY and q.quest_monster == MonsterData.Monsters.CARNIVOROUS_PLANTS:
	...
	elif c.charac_trait == TraitData.Traits.POLLEN_ALLERGY and q.quest_type == QuestData.Quests.PICKING:
		desc_label.text = "I am not sure someone allergic to plants is the best person to handle picking quests..."
	elif c.charac_trait == TraitData.Traits.PRESBYOPIC and q.quest_type == QuestData.Quests.PICKING:
		desc_label.text = "While collecting things, you need to be able to recognize them. Is that obvious?"
	elif c.charac_trait == TraitData.Traits.PRESBYOPIC and q.quest_type == QuestData.Quests.EXCAVATION:
	...
	elif c.charac_trait == TraitData.Traits.HONEST and q.quest_type == QuestData.Quests.ROBBERY:
		desc_label.text = "Even if they need money, asking people to fight against who they are and what they believe is just wrong."
	elif c.charac_trait == TraitData.Traits.HATES_HEAT and q.quest_enviro == EnviroData.Biomes.DESERT:
		desc_label.text = "The average temperature in the desert is 40 degrees. In Winter."
	elif c.charac_trait == TraitData.Traits.HATES_HEAT and q.quest_enviro == EnviroData.Biomes.VOLCANO:
		desc_label.text = "Guess what you can find near a Volcano? Molten lava."
	elif c.charac_trait == TraitData.Traits.SKITTISH and q.quest_enviro == EnviroData.Biomes.MOUNTAIN:
		desc_label.text = "I heard there is perpetual snow in the mountains."
	elif c.charac_trait == TraitData.Traits.SKITTISH and q.quest_enviro == EnviroData.Biomes.CAVE:
	...
	elif c.charac_trait == TraitData.Traits.SUPERNATURAL_HEARING and q.quest_monster == MonsterData.Monsters.CHILDS:
	...
	elif c.charac_trait == TraitData.Traits.ACROPHOBIA and q.quest_enviro == EnviroData.Biomes.VOLCANO:
		desc_label.text = "Cliffs are very high near our volcanos."
	elif c.charac_trait == TraitData.Traits.ACROPHOBIA and q.quest_enviro == EnviroData.Biomes.MOUNTAIN:
		desc_label.text = "Should I explain why sending an acrophobic hero to the mountains was not your best idea?"
	elif c.charac_trait == TraitData.Traits.CLUMSY and q.quest_type == QuestData.Quests.ROBBERY:
		desc_label.text = "While robbing someone, you must be deft if you don't want to be caught."
	elif c.charac_trait == TraitData.Traits.CLUMSY and q.quest_type == QuestData.Quests.EXCAVATION:
		desc_label.text = "Finding the missing object is one thing, but being able to bring it all back is another."
	elif c.charac_trait == TraitData.Traits.CLUMSY and q.quest_type == QuestData.Quests.DELIVERY:
	...
	elif c.charac_trait == TraitData.Traits.ANTROPOPHOBIC and q.quest_type == QuestData.Quests.RESCUE:
		desc_label.text = "Sometimes, rescuing someone is more manageable than resisting the temptation of making them disappear."
	elif c.charac_trait == TraitData.Traits.ANTROPOPHOBIC and q.quest_type == QuestData.Quests.ESCORT:
		desc_label.text = "Escorting someone requires spending time with them. A lot of time. Basically, all day long."
	elif c.charac_trait == TraitData.Traits.ARACNOPHOBIA and q.quest_monster == MonsterData.Monsters.SPIDERS:
		desc_label.text = "The spider and the fly can't make a deal. Same with an arachnophobic."
	elif c.charac_trait == TraitData.Traits.OPHIOPHOBIA and q.quest_monster == MonsterData.Monsters.SNAKES:
		desc_label.text = "It's not clever to send heroes to fight their biggest fear. But you are clever, aren't you?"
	elif c.charac_trait == TraitData.Traits.CHATTY and q.quest_type == QuestData.Quests.ESCORT:
